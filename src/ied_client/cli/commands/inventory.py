...
from ied_client import ENV_PREFIX
...
def from_scd(ctx: CliContext, args) -> CommandResult:
    from ied_client.inventory_ops import from_scd as core_from_scd
    ...
    try:
        inv = core_from_scd(args.scd, experiment=args.experiment, out_path=out_path, protocol=ctx.options.protocol)
...
def _validate_args(p, oneshot: bool) -> None:
    p.add_argument("file", nargs="?", help=f"inventory file (default: --inventory / ${ENV_PREFIX}_INVENTORY)")
...
def validate(ctx: CliContext, args) -> CommandResult:
    from ied_client.inventory_ops import validate as core_validate
