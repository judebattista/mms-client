...
@command(
    "diagnose",
    "Layered connectivity diagnosis: network, protocol layers, identity, model, reports; verdict and next steps",
    area="Diagnosis",
    needs=Needs.TARGET,
...
def diagnose_cmd(ctx: CliContext, args) -> CommandResult:
    ...
    s = ctx.session
    if s is None:
        raise UsageError(f"no device: `connect <device>` first, or run `{TOOL_NAME} diagnose <device>`")
    notes: list[str] = []
    ...
    data = rep.to_json()
    association_layers = {"bind", *s.protocol.association_layers}
    ...
    def text(out: Output) -> None:
        ...
        if all_passed:
            out.ok(f"All {s.protocol.display_name} layers passed.")
        for n in data["notes"]:
            out.note(n)
...
def _discover_args(p, oneshot: bool) -> None:
    ...
    p.add_argument("--no-associate", dest="associate", action="store_false", help="only test the TCP port, do not associate")
    p.add_argument("--allow-large", action="store_true", help="allow networks larger than /22")
    if not oneshot:
        p.add_argument("--port", type=int, help="port (default: the protocol's)")
        p.add_argument("--bind", metavar="IP", help="local IP to scan from")


@command(
    "discover",
    "Scan a subnet for servers of the protocol (sequential, rate-limited), read identity, draft an inventory (DIA-5)",
    area="Diagnosis",
    needs=Needs.NOTHING,
...
def discover_cmd(ctx: CliContext, args) -> CommandResult:
    ...
    protocol = ctx.require_protocol()
    port = getattr(args, "port", None) or ctx.options.port or protocol.default_port
    ...
    try:
        ...
        if status is not None:
            status.stop()
    draft = draft_inventory(devices, protocol.name)
    written = None
    if args.out:
        p = Path(args.out)
        if p.exists():
    ...
    def text(o: Output) -> None:
        ...
        o.table(["address", "TCP", "vendor / model / revision", "logical devices", "error", "release"], rows,
                title=f"{protocol.display_name} servers in {args.subnet} (port {port})")
        if written:
