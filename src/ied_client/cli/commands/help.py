...
from ..shell import protocol_notes
...
def render_explanation(out: Output, exp: Any) -> None:
    try:
        from ied_client.explain.render import render_explanation as plain
...
def explain(ctx: CliContext, args) -> CommandResult:
    ...
    def text(out: Output) -> None:
        try:
            from ied_client.explain.render import render_search
...
def help_cmd(ctx: CliContext, args) -> CommandResult:
    ...
    def text(out: Output) -> None:
        ...
        if ctx.in_shell:
            ...
            out.note(f"One-shot: {TOOL_NAME} [options] <command> <device> [args]. `{TOOL_NAME} --help` lists the options.")
