...
def protocol_notes() -> list[str]:
    """What the installed protocol modules cannot observe (e.g. GOOSE/SV for MMS)."""
    out = []
    for name in protocols.available():
        try:
            note = protocols.get(name).blind_spots
        except Exception:  # pragma: no cover - a broken protocol module must not break the shell
            continue
        if note:
            out.append(note)
    return out
...
class Shell:
    ...
    def banner(self) -> None:
        out = self.ctx.out
        out.line(f"{TOOL_NAME} shell. `help` lists the commands; Tab completes commands and object references.")
