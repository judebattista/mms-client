...
Adding a command
----------------
Drop a module into ``ied_client/cli/commands/`` (every module there is imported automatically) and
decorate a handler::

    from ied_client.cli.registry import Needs, command
...
:class:`~ied_client.core.session.Session` that is not yet associated), or an open association.
"""
...
class Command:
    ...
    def parser(self, *, oneshot: bool) -> CliArgumentParser:
        prog = f"{TOOL_NAME} {self.name}" if oneshot else self.name
        epilog = None
        if self.examples:
...
def load_commands() -> None:
    """Import every module of :mod:`ied_client.cli.commands` (each registers its commands)."""
    global _loaded
    if _loaded:
        return
    _loaded = True
