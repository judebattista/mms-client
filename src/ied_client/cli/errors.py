...
def from_exception(ctx: CliContext, cmd: Command | None, exc: BaseException) -> CommandResult:
    ...
    if isinstance(exc, UnknownProtocolError):
        return failure(codes.tool("unknown-protocol"), str(exc), exit_code=EXIT_USAGE)
...
def catalogue() -> Any:
    ...
    try:
        from ied_client.explain import default_catalogue
...
def hint_context(values: dict[str, Any]) -> Any:
    try:
        from ied_client.explain import HintContext
