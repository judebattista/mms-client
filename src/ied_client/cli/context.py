...
class CliContext:
    ...
    def protocol_module(self) -> ProtocolModule | None:
        """The session's protocol module; without a session the one --protocol names (None: the default)."""
        if self.session is not None:
            return self.session.protocol
        if self.options.protocol:
            try:
                return get_protocol(self.options.protocol)
            except UnknownProtocolError:
                return None
        return None

    def require_protocol(self) -> ProtocolModule:
        """The protocol module for a command without a device (--protocol, else the default)."""
        return get_protocol(self.options.protocol)
    ...
    def make_session(
        ...
        target = resolve_target(
            spec, inv, port=port or o.port, as_client=as_client or o.as_client, local_ip=bind or o.bind, protocol=o.protocol
        )
