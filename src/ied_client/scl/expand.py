"""Expansion of an IED's SCL description into the server model a client will see.
...
control blocks into the instances the device exposes. See :mod:`ied_client.scl.expected`
...
from ied_client.scl.btypes import ValueType, btype_info, value_type_of
from ied_client.scl.edition import edition_of
from ied_client.scl.errors import SclError, location
...
def domain_name(ied_name: str, ld: LDevice) -> str:
    """Name of an LD as a client addresses it: ``ldName`` if present (Ed2), else ``iedName + ldInst``."""
...
class _Expander:
    ...
    def build_da(self, eln: ExpectedLN, d: DADef, fc: str, do_path: tuple[str, ...], da_path: tuple[str, ...],
        ...
        a = ExpectedAttribute(name=d.name, ref=ref, fc=fc, b_type=d.b_type, value_type=value_type_of(d.b_type, d.count),
        ...
        if d.b_type == "Struct":
            ...
            a.value_type = ValueType("array", d.count, ValueType("structure")) if d.count else ValueType("structure")
        elif d.b_type == "Enum":
    ...
    def resolve_fcda(self, own_ld: ExpectedLD, f: FCDA) -> ExpectedFCDA:
        ...
        m = ExpectedFCDA(fcda=f, ld_inst=ld_inst, domain=domain, ln_name=ln_name, fc=f.fc, ref=ref,
                         path=(*do_parts, *da_parts), resolved=False)
        if not f.ld_inst:
    ...
    def build_rcbs(self, eld: ExpectedLD, eln: ExpectedLN, rc: ReportControl, has_owner: bool,
        ...
        for idx, (name, index) in enumerate(names):
            ...
            out.append(ExpectedRCB(
                ...
                buffered=rc.buffered, rpt_id=rc.rpt_id, dat_set_name=rc.dat_set, conf_rev=rc.conf_rev,
