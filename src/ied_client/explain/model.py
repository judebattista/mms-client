...
class HintContext:
    ...
    ``service`` is one of :data:`ied_client.explain.SERVICES`; ``mode`` is ``standard`` or
    ``expert``; ``edition`` is ``Ed1``, ``Ed2``, ``Ed2.1`` or ``unknown``; ``ctl_model`` is a name
    from :data:`ied_client.codes.CTL_MODELS` (a number is converted). ``tags`` carries free-form
