...
Communication, never merged — VER-7), evidence, and the raw exchanges with the device behind it.
"""
...
from ied_client.protocol.registry import UnknownProtocolError
from ied_client.protocol.registry import get as get_protocol
from ied_client.protocol.types import value_to_json
...
class CheckResult:
    ...
    error: ErrorInfo | None = None
    hint: Any = None  # ied_client.explain.Hint, filled in by the caller that owns the catalogue
...
def tool_info(protocol: ProtocolModule | None = None) -> dict[str, Any]:
    """The ``tool`` block: name and version, plus the protocol module's library versions (PROTO-11;
    the default protocol's when no device is involved)."""
    info: dict[str, Any] = {"name": TOOL_NAME, "version": __version__}
    try:
        info.update((protocol or get_protocol()).versions())
...
def default_or_ident() -> str:
    """ORG-3 default originator identification: ``<tool>/<user>@<host>``."""
    try:
        user = getpass.getuser()
    except Exception:
        user = "unknown"
    return f"{TOOL_NAME}/{user}@{socket.gethostname()}"
