"""Read-only file services (FIL-1 … FIL-4), where the protocol module provides them (PROTO-8). There is deliberately no upload or delete (FIL-3):
...
from ied_client.protocol.types import FileEntry
...
def list_files(session: Session, directory: str = "", *, remember_error: bool = True) -> list[FileEntry]:
    ...
    try:
        entries = client.file_directory(directory)
