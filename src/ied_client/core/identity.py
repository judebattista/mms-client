...
class IdentitySource:
    ...
    source: str  # the protocol's identify source (e.g. "mms-identify") | "LPHD.PhyNam" | "LLN0.NamPlt"
    ld: str | None
    reference: str | None
...
def collect_identity(
    ...
    *,
    ...
    rep = IdentityReport()
    # -- the protocol's identify service (PROTO-3)
    src = IdentitySource(protocol.identify_source, None, None)
    try:
    ...
    _decide(rep, override, protocol.identify_source, f"{protocol.display_name} Identify")
...
def _read_struct(client: Any, model: DeviceModel, ref: ObjectRef, source: str, fcs: tuple[str, ...]) -> IdentitySource:
    ...
    for fc in fcs:
        ...
        spec = node.specs[fc]
        try:
            val = client.read(ref.with_fc(fc), spec)
        except ServiceError as e:
            out.error = e.error
            continue
...
def _decide(rep: IdentityReport, override: Any, identify_source: str, identify_label: str) -> None:
    ident = next((s for s in rep.sources if s.source == identify_source), None)
    ...
    candidates = {
        "vendor": [
            (ident.fields.get("vendor"), identify_label) if ident and ident.fields.get("vendor") else None,
            first("vendor", phy),
            first("vendor", nam),
        ],
        "model": [
            (ident.fields.get("model"), identify_label) if ident and ident.fields.get("model") else None,
            first("model", phy),
        ],
        "firmware": [
            (ident.fields.get("revision"), identify_label) if ident and ident.fields.get("revision") else None,
            first("swRev", phy),
            first("swRev", nam),
        ],
    }
