...
from ied_client.protocol.types import RcbValues, Report, VarSpec, format_value, value_to_json
...
def assignments(session: Session, cb: ControlBlockInfo) -> list[str]:
    ...
    if session.inventory is not None:
        for rel in session.inventory.clients_of(session.target.name or ""):
            for r in rel.rcbs:
                if same_rcb(r, cb, session.protocol.names):
                    out.append(rel.client)
    return sorted(set(out))


def same_rcb(text: str, cb: ControlBlockInfo, names: Naming) -> bool:
    """Does an RCB name from the inventory or the command line (``LD/LLN0.BR.brcbA01``, the protocol's own
    name, or the IEC form without FC, ``LD/LLN0.brcbA01``) name this control block?"""
    t = names.control_block_to_iec(text)
...
def find_instances(session: Session, text: str) -> list[ControlBlockInfo]:
    """An RCB reference (``LD/LLN0.BR.brcb01``, or the protocol's own name), a name, or a base name of
    ...
    exact = [
        cb for cb in cands
        if t in (cb.reference, names.control_block(cb.ld, cb.ln, cb.fc, cb.name), cb.name, f"{cb.ln}.{cb.name}")
    ]
    if exact:
        return exact
    base = names.control_block_to_iec(t).split("/")[-1].split(".")[-1]
...
def _members(session: Session, dataset_ref: str | None) -> tuple[list[tuple[str, str | None]], list[VarSpec | None]]:
    ...
    client = session.require_client()
    model = session.model()
    ds = session.protocol.names.parse_dataset(dataset_ref)
    try:
