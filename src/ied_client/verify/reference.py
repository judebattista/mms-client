...
from ied_client.core.identity import SclEdition
...
from ied_client.core.reports import RcbStatus, free_reason, list_rcbs, subscribe
from ied_client.core.results import Category, CheckResult, RawExchange, Status
...
from ied_client.protocol.types import AccessError, DatasetRef, ValueKind, VarSpec, value_to_json
...
class Reference:
    ...
    def from_device(cls, session: Session, choose: str | None = None) -> Reference | None:
        ...
        from ied_client.core.files import find_scl_files, get_file
        ...
        if pick in (None, "none") or pick not in names:
            return None
        tmp = Path(tempfile.mkdtemp(prefix=f"{TOOL_NAME}-scl-"))
    ...
    def checks(self, session: Session, opts: Any) -> Iterator[CheckResult]:
        ...
        yield from _scl_rcb_checks(self.expected, rcbs, session.protocol.names)
...
def _type_matches(expected, live: VarSpec) -> tuple[bool, bool]:
    ...
    mt = expected.value_type
    if mt.kind in ("unknown", None):
        return True, True
    if mt.kind == "array":
        return live.kind is ValueKind.ARRAY, True
    try:
        kind_ok = ValueKind(mt.kind) is live.kind
    except ValueError:
        return True, True
                ...
                for a in do.walk_attributes():
                    ...
                    if not kind_ok:
                        ...
                        yield _r("model-type-mismatch", title, Status.FAIL, CONF, subject=f"{a.ref} [{a.fc}]",
                                 message=f"type {live.type_name()} on the device, {a.value_type} ({a.b_type}) in the reference",
                                 error=codes.check("model-type-mismatch"))
                    elif not size_ok:
                        size_bad += 1
                        yield _r("model-type-mismatch", title, Status.WARN, CONF, subject=f"{a.ref} [{a.fc}]",
                                 message=f"size differs: {live.type_name()} on the device, {a.value_type} in the reference",
                                 error=codes.check("model-type-mismatch"))
            # unexpected DOs
...
# ------------------------------------------------------------------------------------ datasets
def _dataset_of(names: Naming, text: str | None) -> DatasetRef | str | None:
    """A DatSet value as a DatasetRef (the text itself if it does not parse; None if empty)."""
    if not text:
        return None
    try:
        return names.parse_dataset(text)
    except RefError:
        return text


def _native_member(names: Naming, m) -> str:
    try:
        return names.data(m.object_ref)
    except RefError:  # an FCDA without FC: no native name
        return m.ref
...
def _scl_dataset_checks(session: Session, exp: ExpectedServer) -> Iterator[CheckResult]:
    ...
    for ds in exp.datasets:
        dref = ds.dataset_ref
        native = names.dataset(dref)
        if dref.ld not in model.lds or dref not in model.lds[dref.ld].datasets:
            yield _r("dataset-mismatch", title, Status.FAIL, CONF, subject=native, message="dataset is missing on the device",
                     error=codes.check("dataset-mismatch"), certainty="fact")
            continue
        try:
            members, _ = client.dataset_directory(dref)
        except ServiceError as e:
            yield _r("dataset-mismatch", title, Status.FAIL, CONF, subject=native, error=e.error, message=str(e))
            continue
        live = [m.native for m in members]
        want = [_native_member(names, m) for m in ds.members]
        raw = [RawExchange("get-dataset-directory", native, response=live)]
        if live == want:
            yield _r("dataset-mismatch", title, Status.PASS, CONF, subject=native, message=f"{len(live)} members match", raw=raw)
        else:
            ...
            yield _r("dataset-mismatch", title, Status.FAIL, CONF, subject=native, message=msg.strip(),
                     evidence={"device": live, "reference": want}, raw=raw, error=codes.check("dataset-mismatch"))


# ------------------------------------------------------------------------------------ RCBs (RPT-6)
def _scl_rcb_checks(exp: ExpectedServer, rcbs: list[RcbStatus], names: Naming) -> Iterator[CheckResult]:
    ...
    for r in exp.rcbs:
        ...
        want_ds = r.dat_set_dataset
        if _dataset_of(names, v.dataset) != want_ds:
            diffs.append(("DatSet", v.dataset, names.dataset(want_ds) if want_ds else None, False))
...
def _scl_value_checks(session: Session, exp: ExpectedServer, opts: Any) -> Iterator[CheckResult]:
    ...
    active: dict[str, int] = {}
    for sg in exp.sgcbs:
        from ied_client.core import setgroup
    ...
    for items in jobs.values():
        for i in range(0, len(items), 16):
            chunk = items[i : i + 16]
            try:
                vals = client.read_many([a.object_ref for a, _, _ in chunk], [s for _, s, _ in chunk])
            except ServiceError as e:
...
def _scl_identity_checks(session: Session, ref: Reference) -> Iterator[CheckResult]:
    ...
    if ident is not None:
        ...
        from ied_client.core.identity import edition_from_ldns
