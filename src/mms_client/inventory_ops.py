...
from mms_client.inventory import ClientRelation, Device, IdentityOverride, Inventory, Reference
from mms_client.scl import client_relationships, edition_of, expand_server, load_scl, role_hint
...
def from_scd(scd_path: str | Path, *, experiment: str | None = None, out_path: Path | None = None) -> Inventory:
    """Draft inventory from an SCD: one device per IED with an MMS server and an IP address,
    one client relationship per (client IED, server IED) pair from the ClientLN assignments."""
...
def validate(inv: Inventory, *, timeout_s: float = 2.0, associate: bool = True) -> list[ValidationItem]:
    ...
    from mms_client.adapter import ConnectError, IedClient, ServiceError
    ...
    for d in inv.devices:
        ...
        if not associate:
            continue
        c = IedClient(connect_timeout_ms=int(timeout_s * 1000), request_timeout_ms=int(timeout_s * 2000))
        try:
            c.connect(d.ip, d.port)
            ident = c.identify()
            lds = c.get_domain_names()
    ...
    for rel in inv.clients:
        if rel.source_ip:
            try:
                from mms_client.diagnosis.net import is_local_address
