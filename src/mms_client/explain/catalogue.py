...
The catalogue is YAML data (EXP-5). The built-in files live in ``mms_client/data`` and are loaded
in this order:
...
from mms_client.codes import Domain, ErrorInfo
...
_ERROR_TABLES: dict[Domain, Mapping[int, str]] = {
    Domain.IED: codes.IED_ERRORS,
    Domain.MMS: codes.MMS_ERRORS,
    Domain.DATA_ACCESS: codes.DATA_ACCESS_ERRORS,
    Domain.ADD_CAUSE: codes.ADD_CAUSES,
    Domain.CTL_ERROR: codes.CTL_ERRORS,
    Domain.ACSE_DIAG: codes.ACSE_USER_DIAGNOSTICS,
}

# Names every domain must cover (EXP-7 plus the tool/check codes above).
REQUIRED_NAMES: dict[Domain, tuple[str, ...]] = {
    Domain.IED: tuple(codes.IED_ERRORS.values()),
    Domain.MMS: tuple(codes.MMS_ERRORS.values()),
    Domain.DATA_ACCESS: tuple(codes.DATA_ACCESS_ERRORS.values()),
    Domain.ADD_CAUSE: tuple(codes.ADD_CAUSES.values()),
    Domain.CTL_ERROR: tuple(codes.CTL_ERRORS.values()),
    Domain.ASSOCIATION: tuple(codes.ASSOCIATION_OUTCOMES),
    Domain.ACSE_DIAG: tuple(codes.ACSE_USER_DIAGNOSTICS.values()),
    Domain.TOOL: REQUIRED_TOOL_CODES,
    Domain.CHECK: REQUIRED_CHECK_IDS,
}

# `explain <bare-name>`: when a name exists in several domains, prefer them in this order.
BARE_NAME_PREFERENCE: tuple[Domain, ...] = (
    Domain.DATA_ACCESS,
    Domain.IED,
    Domain.MMS,
    Domain.ADD_CAUSE,
...
_DOMAIN_ALIASES: dict[str, Domain] = {
    **{re.sub(r"[^a-z0-9]", "", d.value): d for d in Domain},
    "iederror": Domain.IED,
    "mmserror": Domain.MMS,
    "da": Domain.DATA_ACCESS,
    "dataaccesserror": Domain.DATA_ACCESS,
    "addcause": Domain.ADD_CAUSE,
    "lastapplerror": Domain.CTL_ERROR,
    "ctl": Domain.CTL_ERROR,
    "controlerror": Domain.CTL_ERROR,
    "assoc": Domain.ASSOCIATION,
    "acse": Domain.ACSE_DIAG,
}

# libiec61850 constant prefixes, so that `explain IED_ERROR_ACCESS_DENIED` works.
_CONSTANT_PREFIXES: tuple[tuple[str, Domain], ...] = (
    ("IED_ERROR_", Domain.IED),
    ("DATA_ACCESS_ERROR_", Domain.DATA_ACCESS),
    ("MMS_ERROR_", Domain.MMS),
    ("ADD_CAUSE_", Domain.ADD_CAUSE),
)
...
class _Parser:
    ...
    def check_key(self, where: str, key: str) -> None:
        ...
        try:
            ...
            domains = ", ".join(d.value for d in Domain)
        ...
        if domain in (Domain.TOOL, Domain.CHECK) and not _CODE_NAME_RE.match(name):
            raise self.fail(where, f"key {key!r}: {domain.value} names are lower-case-kebab")
...
def _builtin_sources() -> list[tuple[str, str]]:
    """(label, text) of the built-in catalogue files, in load order."""
    data = importlib.resources.files("mms_client").joinpath("data")
    out: list[tuple[str, str]] = []
    for name in ("hints.yaml", "hints", "glossary.yaml", "glossary"):
        node = data.joinpath(name)
        if name.endswith(".yaml"):
            if not node.is_file():
                raise CatalogueError(f"built-in catalogue file mms_client/data/{name} is missing")
            out.append((f"mms_client/data/{name}", node.read_text(encoding="utf-8")))
        elif node.is_dir():
            for child in sorted(node.iterdir(), key=lambda c: c.name):
                if child.name.endswith(".yaml") and child.is_file():
                    out.append((f"mms_client/data/{name}/{child.name}", child.read_text(encoding="utf-8")))
    return out
...
class Catalogue:
    ...
    # -- query resolution --------------------------------------------------------------------
    def _names_in(self, domain: Domain) -> Iterable[str]:
        table = _ERROR_TABLES.get(domain)
        if table is not None:
            return table.values()
        if domain is Domain.ASSOCIATION:
            return codes.ASSOCIATION_OUTCOMES.keys()
        return sorted(set(self._code_names.get(domain.value, ())) | set(REQUIRED_NAMES.get(domain, ())))

    def _name_in(self, domain: Domain, name: str) -> str | None:
        n = _norm(name)
        if not n:
            return None
    ...
    def _resolve_key(self, q: str) -> str | None:
        ...
        if domain in (Domain.TOOL, Domain.CHECK) and _CODE_NAME_RE.match(rest.casefold()):
            return f"{domain.value}:{rest.casefold()}"  # unknown tool/check code: wildcard entry
        if domain in _ERROR_TABLES and re.fullmatch(r"unknown-\d+", rest.casefold()):
            return f"{domain.value}:{rest.casefold()}"
        return None

    def _resolve_constant(self, q: str) -> str | None:
        upper = q.upper()
        for prefix, domain in _CONSTANT_PREFIXES:
    ...
    def _bare_name_keys(self, q: str) -> list[str]:
        """All keys whose name matches ``q``, in :data:`BARE_NAME_PREFERENCE` order."""
        out: list[str] = []
        for domain in BARE_NAME_PREFERENCE:
            name = self._name_in(domain, q)
            if name is not None:
                out.append(f"{domain.value}:{name}")
        return out
