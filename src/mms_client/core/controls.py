...
from .refs import ObjectRef, RefError, ln_class_of, parse_ref
...
def _control_ref(session: Session, text: str) -> ObjectRef:
    model = session.model()
    ref = parse_ref(text, session.cwd, None, model.ld_names)
...
def infer_cdc(session: Session, ref: ObjectRef, ctl_val: VarSpec, st_val: VarSpec | None) -> tuple[str, str]:
    """CDC from the SCL reference if available, else from the MMS types (CTL-1)."""
    ...
    k = ctl_val.kind
    if k is MmsKind.BOOLEAN and st_val is not None and st_val.kind is MmsKind.BIT_STRING and abs(st_val.size or 0) == 2:
        return "DPC", "inferred from types"
    if k is MmsKind.BOOLEAN:
        return "SPC", "inferred from types"
    if k is MmsKind.INTEGER and st_val is not None and st_val.kind is MmsKind.INTEGER:
        return "INC", "inferred from types (INC and ENC look the same over MMS)"
    if k is MmsKind.BIT_STRING:
        return "BSC", "inferred from types"
    if k in (MmsKind.STRUCTURE, MmsKind.FLOAT):
...
def _read_opt(session: Session, ref: ObjectRef, fc: str) -> tuple[Any, VarSpec | None]:
    ...
    try:
        v = session.require_client().read(ref.ld, ref.mms_item(fc), spec)
    except ServiceError:
        return None, spec
...
def _prepare(session: Session, plan: ControlPlan):
    ...
    try:
        ctl = client.control(plan.ref.iec())
...
def cancel(session: Session, ref_text: str, *, or_cat: int | None = None) -> ControlStepResult:
    ...
    if state is None:
        ...
        client = session.require_client()
        ctl = client.control(ref.iec())
        oc = ensure_or_cat(session, or_cat)
        ctl.configure(or_ident=session.or_ident, or_cat=oc)
    else:
        ctl = session.require_client().control(ref.iec())
    del model
    step = ctl.cancel()
...
def authority_probe(
    ...
    for ref in targets:
        ...
        for c in cats:
            ...
            if row.ctl_model == 2:
                step = ctl.select()
                note = "SBO with normal security: the select carries no orCat"
            else:
...
def _harmless_value(spec: VarSpec | None, current: Value) -> Value:
    ...
    if spec is None:
        return None
    if spec.kind is MmsKind.BOOLEAN:
    ...
    if spec.kind is MmsKind.INTEGER:
