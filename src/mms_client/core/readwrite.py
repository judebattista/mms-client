...
class ReadResult:
    ...
    def to_json(self) -> dict:
        return {
            ...
            "fc": self.ref.fc,
            "mms": f"{self.ref.ld}/{self.ref.mms_item()}",
...
def read(session: Session, text: str | ObjectRef, fc: str | None = None, *, with_qt: bool = True) -> ReadResult:
    ...
    try:
        value = client.read(ref.ld, ref.mms_item(), spec)
    ...
    res = ReadResult(ref, spec, value, duration_s=dt)
...
def _attach_siblings(session: Session, res: ReadResult) -> None:
    ...
    for n in QUALITY_NAMES + TIME_NAMES:
        ...
        if child is not None and res.ref.fc in child.specs:
            items.append(parent.child(n).mms_item(res.ref.fc))
            names.append(n)
    if not items:
        return
    try:
        vals = session.require_client().read_multiple(res.ref.ld, items)
    except ServiceError:
        return
...
def write(
    ...
    *,
    ...
    before = client.read(ref.ld, ref.mms_item(), spec)
    res = WriteResult(ref, spec, before, value, warnings=warnings)
    if confirm:
        summary = (
    ...
    t0 = time.perf_counter()
    try:
        client.write(ref.ld, ref.mms_item(), spec, value)
        res.ok = True
    except ServiceError as e:
        res.error = e.error
