...
from .refs import ObjectRef, RefError, parse_ref
...
def show(session: Session, ld: str | None = None) -> list[SgcbState]:
    ...
    for cb in sgcbs(session):
        ...
        st = SgcbState(cb)
        try:
            val = client.read(cb.ld, f"{cb.ln}$SP$SGCB", cb.spec)
...
def edit(
    ...
    *,
    ...
    for text, value_text in assignments:
        ref = parse_ref(text, session.cwd, None, model.ld_names).with_fc("SE")
        try:
    ...
    try:
        ...
        befores = [client.read(r.ld, r.mms_item(), s) for r, s, _ in parsed]
        if confirm:
        ...
        for (r, s, v), b in zip(parsed, befores, strict=True):
            ...
            try:
                client.write(r.ld, r.mms_item(), s, v)
                item["ok"] = True
            except ServiceError as e:
        ...
        for item, (r, s, v) in zip(res.items, parsed, strict=True):
            after = client.read(r.ld, r.mms_item(), s)
