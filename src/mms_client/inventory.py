"""Rack inventory: one YAML file per experiment (INV-1 … INV-5).
...
class Device:
    ...
    def to_yaml(self, base: Path | None) -> dict:
        d: dict[str, Any] = {"name": self.name, "ip": self.ip, "port": self.port, "role": self.role}
        if self.reference:
...
def parse_inventory(data: dict[str, Any], path: Path | None = None) -> Inventory:
    ...
    for i, d in enumerate(data.get("devices") or []):
        ...
        extra = {k: v for k, v in d.items() if k not in ("name", "ip", "port", "role", "reference", "identity", "security")}
        inv.devices.append(
            Device(
                name=name,
                ip=_ip(d["ip"], where + ".ip"),
                port=int(d.get("port", 102)),
                role=role,
                reference=ref,
