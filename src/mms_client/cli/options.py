...
SESSION_OPTIONS = ("inventory", "expert", "as_client", "bind", "port", "log_dir", "no_log", "timeout")
...
SHELL_ALTERNATIVES = {
    ...
    "--inventory": "the inventory is chosen when the shell starts (mms-client -i FILE shell <device>)",
    "-i": "the inventory is chosen when the shell starts (mms-client -i FILE shell <device>)",
...
class Options:
    ...
    def from_namespace(cls, ns: argparse.Namespace) -> Options:
        ...
        if o.inventory is None:
            o.inventory = os.environ.get("MMS_CLIENT_INVENTORY") or None
        return o
...
def _add_line_options(p: argparse.ArgumentParser) -> None:
    ...
    c.add_argument("--or-ident", dest="or_ident", metavar="TEXT", default=None, help="originator identification (default mms-client/<user>@<host>)")
...
def global_parser() -> CliArgumentParser:
    ...
    p = CliArgumentParser(prog="mms-client", add_help=False)
    s = p.add_argument_group("session")
    s.add_argument("-i", "--inventory", metavar="FILE", default=None, help="experiment inventory (default: $MMS_CLIENT_INVENTORY)")
    ...
    s.add_argument("--port", type=int, metavar="N", default=None, help="MMS port (default 102 or the inventory's)")
    s.add_argument("--log-dir", dest="log_dir", metavar="DIR", default=None, help="session log directory (default ~/.local/state/mms-client/sessions)")
