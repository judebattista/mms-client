...
from mms_client.core.results import envelope, jsonable
...
def lookup(tokens: list[str], *, in_shell: bool) -> tuple[Command, list[str]]:
    ...
    if cmd.shell_only and not in_shell:
        raise UsageError(f"`{cmd.name}` only makes sense inside the shell (mms-client shell <device>)")
    return cmd, tokens[n:]
...
def to_envelope(ctx: CliContext, cmd_name: str, result: CommandResult) -> dict[str, Any]:
    ...
    return envelope(cmd_name, result.data, device=ctx.device_label, ok=result.ok, **extra)
