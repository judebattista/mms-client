...
from mms_client.core import restore as core_restore
from mms_client.core.controls import check_or_cat_allowed, parse_or_cat
from mms_client.core.results import default_or_ident
from mms_client.core.safety import Mode, Policy
...
def export(ctx: CliContext, args) -> CommandResult:
    from mms_client.core.incident import build_incident, write_incident

    s = ctx.session
    if s is None:
        raise UsageError("no device: `connect <device>` first, or run `mms-client export <device> --incident FILE`")
