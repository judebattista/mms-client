...
def ls(ctx: CliContext, args) -> CommandResult:
    ...
    if r is not None and r.kind == "ln" and r.ln is not None:
        ...
        prefix = f"{r.ln.name}$"
        data["datasets"] = [d[len(prefix) :] for d in r.ld.datasets if d.startswith(prefix)]
...
def describe(ctx: CliContext, args) -> CommandResult:
    s = ctx.require_session()
    model = s.model()
    ref = parse_ref(args.ref, s.cwd, None, model.ld_names)
    r = model.resolve(ref)
    ...
    if r.kind == "ld":
        ...
        for fc, spec in node.specs.items():
            if ref.fc and fc != ref.fc:
                continue
            fcs[fc] = {"type": spec.type_name(), "mms": f"{ref.ld}/{ref.mms_item(fc)}", **writability(fc, spec.is_basic)}
    ...
    def text(out: Output) -> None:
        ...
        for fc, f in data.get("fcs", {}).items():
            rows.append((f"[{fc}]", f"{f['type']}   MMS name {f['mms']}"))
...
def _cdc_of(s, ref: ObjectRef, node: DataNode, is_do: bool) -> dict[str, str] | None:
    ...
    return {"cdc": "unknown", "source": "not visible over MMS without an SCL reference"}


# ---------------------------------------------------------------------------------- dataset
def _dataset_args(p, oneshot: bool) -> None:
    p.add_argument("name", nargs="?", help="dataset: LD/LLN0.Events, LLN0$Events, or just Events (default: list all)")


def find_dataset(model: DeviceModel, cwd: tuple[str, ...], text: str) -> tuple[str, str]:
    """(domain, MMS name) of a dataset given in any accepted form."""
    t = text.strip()
    if "/" in t:
        return dataset_ref_to_mms(t)
    item = t.replace(".", "$")
    if "$" in item and cwd:
        return cwd[0], item
    matches = [(ld, d) for ld, ldi in model.lds.items() for d in ldi.datasets if d == item or d.split("$")[-1] == item]
    if cwd:
        local = [m for m in matches if m[0] == cwd[0]]
        matches = local or matches
    if len(matches) == 1:
        return matches[0]
    if not matches:
        raise NotInModelError(f"no dataset {text!r} (use `dataset` to list them)")
    raise RefError(f"{text!r} matches several datasets: " + ", ".join(f"{ld}/{d.replace('$', '.')}" for ld, d in matches))
...
def dataset(ctx: CliContext, args) -> CommandResult:
    ...
    if not args.name:
        items = [
            {"reference": f"{ld}/{d.replace('$', '.')}", "mms": f"{ld}/{d}"} for ld, ldi in model.lds.items() for d in ldi.datasets
        ]
        return CommandResult(
            data={"datasets": items},
    ...
    data = {"dataset": ref_text, "mms": f"{domain}/{item}", "deletable": deletable, "members": rows, "unresolved": len(unresolved)}

    def text(out: Output) -> None:
        out.table(
            ["#", "member", "FC", "type", "resolves"],
            [(r["index"], r["reference"] or r["mms"], r["fc"] or "", r.get("type", ""), "yes" if r["resolves"] else f"NO: {r.get('reason', '')}")
             for r in rows],
