...
def _connect_args(p, oneshot: bool) -> None:
    if not oneshot:
        ...
        p.add_argument("--port", type=int, metavar="N", help="MMS port")
...
def association_info(ctx: CliContext, duration_s: float | None = None) -> dict[str, Any]:
    ...
    params = None
    try:
        params = s.require_client().connection_params().to_json()
...
def _render_association(out: Output, d: dict[str, Any], *, title: str) -> None:
    ...
    if p:
        rows.append(("max PDU size", p["max_pdu_size"]))
        rows.append(("outstanding calls", f"calling {p['max_serv_outstanding_calling']}, called {p['max_serv_outstanding_called']}"))
        rows.append(("nesting level", p["data_structure_nesting_level"]))
        rows.append(("services", ", ".join(p["services_supported"])))
...
def connect(ctx: CliContext, args) -> CommandResult:
    ...
    def text(out: Output) -> None:
        ...
        if not ctx.in_shell:
            _render_association(out, data, title="Association")
    ...
    def text(out: Output) -> None:
        ...
        if ident["edition_evidence"]:
            ...
            for ev in ident["edition_evidence"]:
                out.line(f"  - {ev}")
        _render_association(out, data["association"], title="Association")
        out.line(
