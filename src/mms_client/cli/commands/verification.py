...
def _ignore(path: str | None) -> Any:
    from mms_client.verify.diff import IgnoreRules
...
def check(ctx: CliContext, args) -> CommandResult:
    ...
    def text(out: Output) -> None:
        ...
        out.note("GOOSE and Sampled Values are not MMS: these checks cannot see them.")
...
def diff(ctx: CliContext, args) -> CommandResult:
    from mms_client.verify.diff import diff_snapshots
    from mms_client.verify.snapshot import capture
