...
from mms_client.core import reports
from mms_client.core.reports import NoFreeRcbError, RcbStatus, Subscription, free_reason
...
def _find_sub(ctx: CliContext, text: str | None) -> Subscription | None:
    ...
    for sub in subs:
        cb = sub.cb
        if text in (cb.reference, cb.mms_reference, cb.name, f"{cb.ln}.{cb.name}") or cb.name.startswith(text):
            return sub
    return None
