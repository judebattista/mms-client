...
from mms_client.adapter import value_from_json
from mms_client.core import readwrite, setgroup
from mms_client.core.readwrite import ReadResult, describe_quality
...
def render_read(out: Output, res: ReadResult, data: dict[str, Any]) -> None:
    out.parts((ref_label(res.ref.iec(), res.ref.fc), STYLE_REF), (f"   MMS {data['mms']}", STYLE_NOTE))
...
def read(ctx: CliContext, args) -> CommandResult:
    ...
    data = res.to_json()
    result = CommandResult(data=data, text=lambda out: render_read(out, res, data))
    if res.error is not None:
        result.ok = False
