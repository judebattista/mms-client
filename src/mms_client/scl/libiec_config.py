...
* ``<type>`` from :mod:`mms_client.scl.btypes` (``BTypeInfo.libiec_type``), ``<fc>`` from
  ``mms_client.codes.FCS``.
...
from mms_client.scl.btypes import LIBIEC_CONSTRUCTED, btype_info
...
class _Writer:
    ...
    def da(self, a: ExpectedAttribute) -> None:
        ...
        if info is None or info.libiec_type is None:
            self.warn(f"{a.ref}: bType {a.b_type!r} cannot be modelled in libiec61850; attribute left out")
            return
        head = f"DA({a.name} {a.count} {info.libiec_type} {fc} {trg} {s_addr})"
        if a.count:
            self.out(head + "{")
            for i in range(a.count):
