...
from ..codes import data_access_error, ied_error, mms_error
from . import _native
from ._native import c_str, iter_linked_list, lib, py_str, string_list
from .codec import decode, encode, spec_from_native
from .types import MMS_KIND_BY_NUMBER, RCB_ELEMENTS, ConnectionParams, VariableListEntry
...
def _check_mms(service: str, target: str | None, err: C.c_int) -> None:
    if err.value != 0:
        raise ServiceError(service, target, mms_error(err.value))
...
class IedClient:
    ...
    def connect(self, host: str, port: int = 102, *, local_ip: str | None = None, local_port: int = 0) -> None:
        ...
        with self._lock:
            if self._con is not None:
                raise ServiceError("associate", host, ied_error(2))
            con = L.IedConnection_create()
            if not con:
                raise ConnectError("associate", host, ied_error(99), "could not allocate a connection")
            ...
            if err.value != 0:
                ...
                raise ConnectError("associate", f"{host}:{port}", ied_error(err.value))
            self._con = con
    ...
    def identify(self) -> ServerIdentity:
        ...
        if not p:
            raise ServiceError("identify", None, mms_error(9), "empty response")
        try:
            s = p.contents
    ...
    def get_variable_spec(self, domain: str, item: str) -> VarSpec:
        ...
        try:
            ...
            if not p:
                raise ServiceError("get-variable-access-attributes", f"{domain}/{item}", mms_error(9), "empty")
            return spec_from_native(p)
        finally:
            if p:
                L.MmsVariableSpecification_destroy(p)

    def get_dataset_directory(self, domain: str, name: str) -> tuple[list[VariableListEntry], bool]:
        ...
        members: list[VariableListEntry] = []
        try:
    ...
    def read(self, domain: str, item: str, spec: VarSpec | None = None) -> Value:
        ...
        try:
            ...
            if not p:
                raise ServiceError("read", f"{domain}/{item}", mms_error(9), "empty response")
    ...
    def read_multiple(
        ...
        try:
            ...
            if not p:
                raise ServiceError("read", domain, mms_error(9), "empty response")
            n = L.MmsValue_getArraySize(p)
            out: list[Value] = []
            for i in range(len(items)):
                if i >= n:
                    out.append(AccessError(data_access_error(12)))
                    continue
    ...
    def write(self, domain: str, item: str, spec: VarSpec, value: Value) -> None:
        ...
        if dae >= 0:
            raise ServiceError("write", target, data_access_error(dae))
    ...
    def get_rcb(self, reference: str) -> RcbValues:
        ...
        try:
            ...
            if not rcb:
                raise ServiceError("get-rcb", reference, ied_error(99), "empty response")
        ...
        try:
            ...
            for key, val in changes.items():
                ...
                if key == "rpt_id":
                    ...
                    entry_val = encode(VarSpec(ValueKind.OCTET_STRING, size=8), raw)
...
class ControlObject:
    ...
    def _step(self, service: str, call: Callable[[], bool], ctl_num: int) -> ControlStepResult:
        ...
        ied = ied_error(int(L.ControlObjectClient_getLastError(self._ptr())))
        fresh = after != before
        return ControlStepResult(
            service=service,
            ok=ok,
            duration_s=dt,
            ied_error=ied if not ok else ied_error(0),
