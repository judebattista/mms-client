...
from ied_client.protocol.types import ValueKind, VarSpec
...
def model() -> DeviceModel:
    ...
    cswi = S(
        "CSWI1",
        S("ST", S("Pos", B("stVal", ValueKind.BIT_STRING, 2), q, t), S("Loc", B("stVal"), q, t)),
        S("CO", S("Pos", S("Oper", B("ctlVal"), B("ctlNum", ValueKind.UNSIGNED, 8)))),
        S("CF", S("Pos", B("ctlModel", ValueKind.INTEGER, 8), B("sboTimeout", ValueKind.UNSIGNED, 32))),
    )
    ggio = S(
        "GGIO1",
        S("MX", S("AnIn1", S("mag", B("f", ValueKind.FLOAT, 32)), q, t)),
        S("SP", S("Setp1", B("setVal", ValueKind.INTEGER, 32))),
    )
    lln0 = S(
        "LLN0",
        S("ST", S("Mod", B("stVal", ValueKind.INTEGER, 8), q, t)),
        S("RP", S("urcbA01", B("RptID", ValueKind.VISIBLE_STRING, -65))),
    )
